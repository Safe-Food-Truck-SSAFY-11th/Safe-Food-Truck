  ...
  useEffect(() => {
    ...
    script.onload = () => {
      ...
      window.kakao.maps.load(() => {
        ...
        // 지도 최초에 렌더링 할 때 표시할 중심 좌표 설정하는 메서드입니당.
        const center = new window.kakao.maps.LatLng(userLocation?.latitude ,userLocation?.longitude)

        // 지도가 이동, 확대, 축소로 인해 중심좌표가 변경되면 마지막 파라미터로 넘어온 함수를 호출하도록 이벤트를 등록
        window.kakao.maps.event.addListener(map, 'center_changed', function() {
          
          // 지도의  레벨을 가져옴 // 현재 불필요하긴 한데 필요 할 때 사용 하려고 합니다 
