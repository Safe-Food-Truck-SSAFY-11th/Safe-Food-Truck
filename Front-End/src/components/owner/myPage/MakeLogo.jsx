...
const MakeLogo = ({ closeMakeLog, translateResult, storeId }) => {
  ...
  const getImage = async (param) => {
    ...
    const response = await axiosInstance.post(`/logos`, {
      logoStyle: param
    });
  ...
  const saveImage = async () => {
    await axiosInstance.patch("/stores/ai-logo", {
      storeId: storeId,         // 업데이트할 스토어의 ID
      "savedUrl": aiImageURL,   // 새로운 이미지 URL
      "savedPath": "empty"               // 새로운 이미지 경로
    });
  ...
  const handleSubmit = (event) => {
    ...
    var param = translateResult + ", " + selectedOptions.join(", ") + ", store logo style";
  ...
  return (
    <div className={styles.makeLogoModal}>
      <div className={styles.modalContent}>
        ...
        <div>
          <button className={styles.moveButton} onClick={saveImage}>
            저장 하기
          </button>
