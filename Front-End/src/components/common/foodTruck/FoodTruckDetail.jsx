...
function FoodTruckDetail() {
  ...
  useEffect(() => {
    ...
    if (pastOrders?.customerOrderResponseDtos) {
      const meet = pastOrders.customerOrderResponseDtos.some(
        (order) => order.storeId === Number(storeId)
      );
      if (meet) {
        setIsMeet(true);
      }
    ...
    <div>
      ...
      <div className={styles.foodTruckDetail}>
        ...
        <div className={styles.content}>
          {view === "menu" && (
            <FoodTruckMenuList menus={menus} storeId={storeId} />
          )}
          {view === "info" && (
            <FoodTruckInfo
              truck={selectedTruck}
              reviews={reviews}
              isMeet={isMeet}
              selectedTruckSchedule={selectedTruckSchedule}
            />
          )}
